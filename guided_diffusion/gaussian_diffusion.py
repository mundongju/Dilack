...
import ADMM_Torch_color_update #moldify
import ADMM_Torch_color
#### ADMM-tweedie plug in ##### 2025-07-11 ############
...
class GaussianDiffusion:
    def __init__(self,
        ...
        print ('self.sqrt_alphas_cumprod',self.sqrt_alphas_cumprod.shape)
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            img, distance, s_theta, DPS_grad = measurement_cond_fn(x_t=out['sample'],
                                      measurement=measurement,
                                      noisy_measurement=noisy_measurement,
                                      x_prev=img,
                                      x_0_hat=out['pred_xstart'],s_theta=out['s_theta'])
            img = img.detach_()
    ...
    def p_sample_loop_sdedit(self,
                    model,
                    x_start,
                    measurement,
                    measurement_cond_fn,deconv_guide,
                    record,
                    save_root):
        """
        The function used for sampling from noise.
        ...
        for idx in pbar:
            ...
            img, distance, s_theta, DPS_grad = measurement_cond_fn(x_t=out['sample'],
                                        measurement=measurement,
                                        noisy_measurement=noisy_measurement, deconv_guide =deconv_guide,
                                        x_prev=img,
                                        x_0_hat=out['pred_xstart'],s_theta=out['s_theta'])
            img = img.detach_()
    ...
    #ver3_rev- p_sample_loop_deconv --> p_sample_loop_ADMM_update_new
    def p_sample_loop_ADMM_update_new(self,
                      model,
                      x_start,
                      measurement_org,
                      measurement,
                      measurement_cond_fn,admm_kernel,pad_size,skip_step, #modify
                      record,
                      save_root):
        """
        The function used for sampling from noise.
        ...
        for idx in pbar:
            ...
            #X_0 hat plus
            # breakpoint()
            x_0_hat_inp = out['pred_xstart'].clone()
            
            # x_0_hat_inp = (1+x_0_hat_inp)/2 #[0,1]
            
            # Give condition.
            noisy_measurement = self.q_sample(measurement, t=time)
            idx_2 = 999-idx
            if idx_2 == 0:
                x_0_hat_inp = torch.zeros_like(x_0_hat_inp).to(x_0_hat_inp.device)
            #ADMM update per each step
            # print(x_0_hat_inp)
            if idx_2 % skip_step == 0: #modify
                call_ADMM += 1
                print("ADMM update: !!", )
                print("Call-ADMM : ", call_ADMM)
                print(idx_2)
                CLAMP = 1e-3
    
                A = ADMM_Torch_color_update.ADMM_LGE(device=device, #modify
                                                iterations=1000,
                                                stacks=1,
                                                psf=admm_kernel,
                                                infer=True,
                                                ...
                                                clamp_=CLAMP,
                                                     x_0_hat = x_0_hat_inp,
                                                     call_ADMM = call_ADMM,
                                                     Total_call = 1000/skip_step,
                                                )
                # measurement_org = 2*measurement_org - 1
                admm_img = A.forward(measurement_org)
                ...
                # print(deconv_guide)
                # plt.imsave("./admm_test_x_0_norm0001_call_num20_update_{}.png".format(idx_2), clear_color(deconv_guide))
    ...
    def p_sample(self, model, x, t, mask):
    ...
    def p_mean_variance(self, model, x, t):
        model_output = model(x, self._scale_timesteps(t)) #여기에 masking씌워보기
...
class DDPM(SpacedDiffusion):
    def p_sample(self, model, x, t):
        out = self.p_mean_variance(model, x, t) #x_0 hat 수식
        sample = out['mean']
        # S_theta 가져오려고 (03/26)
        s_theta = out['s_theta']
        ...
        # S_theta 가져오려고 (03/26)
        return {'sample': sample, 'pred_xstart': out['pred_xstart'], 's_theta':s_theta}
