...
class ConditioningMethod(ABC):
    ...
    # Dilack Gradient
    def grad_and_value_ROI_pat_KL_stoch(self, x_prev, x_0_hat, measurement,deconv_guide,deconv_scale,patch_size,threshold_init,skip_point,YS, **kwargs):
...
class PosteriorSamplingROI(ConditioningMethod):
    ...
    def conditioning(self, x_prev, x_t, x_0_hat, measurement,deconv_guide,s_theta, **kwargs):
        s_theta = s_theta
        norm_grad, norm_total,arg_norm_true, arg_trans_S = self.grad_and_value_ROI_pat_KL_stoch(x_prev=x_prev, x_0_hat=x_0_hat, measurement=measurement,deconv_guide=deconv_guide,deconv_scale=self.deconv_scale,patch_size=self.patch_size, threshold_init=self.threshold_init,skip_point = self.skip_point,YS=self.YS,**kwargs)
        ...
        if arg_norm_true:
            ...
            if self.num_call_ind <= 200:
                ...
                if arg_trans_S:
                    ...
                    x_t -= norm_grad * self.scale
                    print("guiding mode!")
            print("conditional diffusion- something full step + remaining SDEdit!")
