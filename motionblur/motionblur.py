...
class Kernel(object):
    ...
    def kernelMatrix(self) -> np.ndarray:
        ...
        if self._kernel_matrix is None:
            # generate kernel if needed
            self._createKernel()
            self._kernel_matrix  = np.asarray(self.kernel_image, dtype=np.float32)
    ...
    @kernelMatrix.setter
    def kernelMatrix(self, kernel: np.ndarray):
        """Sets the kernel matrix."""
        if not isinstance(kernel, np.ndarray):
            raise ValueError("Kernel matrix must be a numpy ndarray")
        if kernel.shape != self.SIZE:
            raise ValueError(f"Kernel matrix must be of shape {self.SIZE}")
        self._kernel_matrix = kernel
